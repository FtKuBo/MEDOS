...
struct idt_entry_structer idt_entries[256];
struct idt_ptr_structer idt_ptr;

extern void idt_flush(uint32_t);

void initIdt(){
    idt_ptr.limit = sizeof(struct idt_entry_structer) * 256 - 1;
    idt_ptr.base = (uint32_t) &idt_entries;

    memset(&idt_entries, 0, sizeof(struct idt_entry_structer)*256);
...
void setIdtGate(int8_t num, uint32_t base, uint16_t selector, uint8_t flags){
